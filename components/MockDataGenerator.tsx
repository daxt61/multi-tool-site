...
export function MockDataGenerator({ initialData, onStateChange }: { initialData?: any; onStateChange?: (state: any) => void }) {
  ...
  }, [count, format, locale, onStateChange]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-end">
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                <span className="text-[10px] font-bold text-slate-400 uppercase">{t('mockdata.locale')}</span>
                <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
                  <button
          ...
          <button
            ...
            className="w-full h-[60px] bg-indigo-600 text-white rounded-2xl font-black text-lg shadow-xl shadow-indigo-600/20 hover:bg-indigo-700 transition-all active:scale-95 flex items-center justify-center gap-2 focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none"
          >
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex justify-between items-center px-1">
            <div className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-400">
              ...
              {t('mockdata.result')} ({format.toUpperCase()})
            </div>
          ...
          <textarea
            readOnly
            value={outputText}
            className="w-full h-96 p-8 bg-slate-900 text-indigo-300 border border-slate-800 rounded-[2.5rem] font-mono text-sm leading-relaxed resize-none outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/20"
          />

          {/* Table Preview */}
          <div className="pt-8 space-y-4">
             <div className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-400">
                <Table className="w-4 h-4" /> {t('mockdata.preview')}
             </div>
