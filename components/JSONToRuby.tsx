    ...
    try {
      ...
      const getRubyType = (val: any, fieldName: string, depth: number): string => {
        ...
        if (typeof val === 'object') {
          ...
          // Recurse to generate types/classes for nested objects
          ...
          if (fields.length > 0) {
            classStr += `  attr_accessor ${fields.join(', ')}\n\n`;
            classStr += `  def initialize(${fieldEntries.map(([k]) => `${toSnakeCase(k)}:`).join(', ')})\n`;
            fieldEntries.forEach(([k]) => {
              classStr += `    @${toSnakeCase(k)} = ${toSnakeCase(k)}\n`;
            });
            classStr += `  end\n`;
          } else {
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-1">
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="ruby-output" className="text-xs font-black uppercase tracking-widest text-slate-400 cursor-pointer">Ruby Classes</label>
            </div>
      ...
      <div className="bg-indigo-50 dark:bg-indigo-900/10 p-8 rounded-[2.5rem] border border-indigo-100 dark:border-indigo-900/20 flex items-start gap-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
            {t('jsontoruby.about_text', 'Cet outil génère des classes Ruby avec des accesseurs de données (attr_accessor) à partir de vos données JSON. Il gère les types de base et les objets imbriqués, en suivant les conventions de nommage Ruby (snake_case pour les méthodes et variables, PascalCase pour les classes).')}
          </p>
        </div>
      </div>
    </div>
