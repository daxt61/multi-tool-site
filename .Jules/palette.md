...
## 2026-02-16 - [Tactile Feedback & Navigation Affordance]
**Learning:** Adding `active:scale-95` to primary action buttons (like favorites and filters) provides immediate tactile feedback that makes the interface feel more responsive. Furthermore, animating navigation links (e.g., sliding a back arrow on hover) provides a delightful micro-interaction that reinforces the user's mental model of the app structure.
**Action:** Standardize `active:scale-95` for interactive buttons and use directional animations for navigation links.
