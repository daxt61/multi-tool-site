    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center px-1">
          <label htmlFor="case-textarea" className="text-xs font-black uppercase tracking-widest text-slate-400">
            Votre texte
          </label>
          <button
            onClick={() => setText('')}
        ...
        <textarea
          ...
          className="w-full h-32 p-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all dark:text-slate-300"
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {Object.entries(conversions).map(([name, converter]) => {
          ...
          return (
            <div key={name} className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-2xl p-4">
              ...
              <div className="bg-slate-50 dark:bg-slate-950 p-3 rounded-xl font-mono text-sm min-h-[3rem] break-all border border-slate-100 dark:border-slate-900 flex items-center dark:text-slate-300">
                {converted || <span className="text-slate-400">Résultat...</span>}
              </div>
            </div>
          );
        })}
