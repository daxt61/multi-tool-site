...
export function JSONToMongoose({ initialData, onStateChange }: { initialData?: any; onStateChange?: (state: any) => void }) {
  ...
  const handleConvert = useCallback(() => {
    try {
      ...
      const schemas: { name: string; schema: string }[] = [];
      const schemaNames = new Set<string>();

      const getMongooseType = (val: any, fieldName: string, depth: number): string => {
        if (depth > MAX_DEPTH) return 'Schema.Types.Mixed';
        if (val === null || val === undefined) return 'Schema.Types.Mixed';

        if (Array.isArray(val)) {
          const itemType = val.length > 0 ? getMongooseType(val[0], fieldName, depth + 1) : 'Schema.Types.Mixed';
          return `[${itemType}]`;
        }
        ...
        if (typeof val === 'object') {
          const subFields = Object.entries(val).map(([key, value]) => {
            ...
            const safeKey = isValidIdent ? key : JSON.stringify(key).replace(/<\/script>/g, '<\\/script>');
    ...
    <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-1">
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="mongoose-output" className="text-xs font-black uppercase tracking-widest text-slate-400 cursor-pointer">Mongoose Schema</label>
            </div>
          ...
          <textarea
            ...
            value={output}
            readOnly
            placeholder={t('jsontomongoose.placeholder')}
      ...
      <div className="bg-indigo-50 dark:bg-indigo-900/10 p-8 rounded-[2.5rem] border border-indigo-100 dark:border-indigo-900/20 flex items-start gap-4">
        ...
        <div className="space-y-2">
          <h4 className="font-bold dark:text-white">{t('jsontomongoose.about_title')}</h4>
          <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
            {t('jsontomongoose.about_text')}
          </p>
        </div>
      </div>
    </div>
