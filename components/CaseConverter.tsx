    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="space-y-4">
        ...
        <textarea
          ...
          className="w-full h-48 p-6 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all text-lg leading-relaxed dark:text-slate-300 resize-none"
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {Object.entries(conversions).map(([name, converter]) => {
          ...
          return (
            <div key={name} className="group bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 hover:border-indigo-500/50 transition-all">
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  ...
                  disabled={!text}
                  className={`p-2 rounded-lg transition-all ${copied === name ? 'bg-emerald-500 text-white' : 'text-slate-400 hover:text-indigo-500 bg-slate-50 dark:bg-slate-800'}`}
              ...
              <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-xl font-mono text-sm min-h-[3rem] break-all dark:text-slate-300 flex items-center">
                {converted || <span className="text-slate-400 italic text-xs uppercase tracking-widest font-black">En attente...</span>}
              </div>
            </div>
          );
        })}
