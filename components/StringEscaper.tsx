...
import { Type, Copy, Check, Trash2, ArrowLeftRight } from 'lucide-react';
...
export function StringEscaper({ initialData, onStateChange }: { initialData?: any; onStateChange?: (state: any) => void }) {
  ...
  const handleClear = useCallback(() => {
    setInput('');
    setOutput('');
    toast.success(t('stringescaper.reset_success'));
    setTimeout(() => inputRef.current?.focus(), 0);
  }, [t]);

  // Stable handlersRef listener pattern
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Allow Escape to clear even if focused on the input
      if (e.key === 'Escape') {
        e.preventDefault();
        handleClear();
        return;
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-slate-50 dark:bg-slate-900/50 p-6 rounded-3xl border border-slate-200 dark:border-slate-800">
        ...
        <div className="flex items-center gap-4">
          ...
          <select
            ...
            className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 font-bold text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-1">
            <div className="flex items-center gap-2">
              <ArrowLeftRight className="w-4 h-4 text-emerald-500" aria-hidden="true" />
            ...
            <button
              onClick={handleCopy}
              disabled={!output}
              className={`text-xs font-bold px-3 py-1 rounded-full transition-all flex items-center gap-2 border focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none ${
                copied
                  ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20'
                  : 'text-slate-500 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed border-transparent'
              }`}
            ...
            >
              ...
              {!copied && <Kbd modifier={null} className="bg-white/50 border-slate-200 dark:border-slate-700 text-slate-600">C</Kbd>}
            </button>
          </div>
          <textarea
            id="string-output"
