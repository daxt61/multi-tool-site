      ...
      {!hasFile ? (
        ...
        >
          <input
            ...
            className="hidden"
          />
          <div className="w-20 h-20 bg-slate-100 dark:bg-slate-800 rounded-3xl flex items-center justify-center mx-auto mb-6 text-slate-600 group-hover:text-indigo-500 group-hover:scale-110 transition-all">
            <Upload className="w-10 h-10" />
          </div>
          <h3 className="text-2xl font-black mb-2 dark:text-white">Glissez un fichier ici</h3>
          <p className="text-slate-500 dark:text-slate-600">ou cliquez pour parcourir vos fichiers</p>
          <p className="text-xs font-bold text-slate-600 mt-8 uppercase tracking-widest">Tous types de fichiers supportés (Max 5MB)</p>
        </div>
      ) : (
              ...
              <div className="aspect-square relative flex items-center justify-center bg-white dark:bg-slate-950 p-4">
                {isImage ? (
                  ...
                  <div className="flex flex-col items-center gap-4 text-slate-300">
                    ...
                    <span className="text-xs font-bold uppercase tracking-widest text-slate-600 text-center px-4">Aperçu non disponible pour ce type de fichier</span>
                  </div>
                )}
              </div>
              <div className="p-6 space-y-4">
                <div className="space-y-1">
                  <div className="text-[10px] font-black uppercase tracking-widest text-slate-600">Nom du fichier</div>
                  <div className="text-sm font-bold truncate dark:text-slate-200">{fileInfo?.name}</div>
                </div>
                <div className="flex justify-between">
                  <div className="space-y-1">
                    <div className="text-[10px] font-black uppercase tracking-widest text-slate-600">Taille</div>
                    <div className="text-sm font-bold dark:text-slate-200">{fileInfo?.size}</div>
                  </div>
                  <div className="space-y-1 text-right">
                    <div className="text-[10px] font-black uppercase tracking-widest text-slate-600">Format</div>
          ...
          <div className="lg:col-span-8 space-y-4">
            ...
            <div className="relative group">
              <textarea
                readOnly
                value={base64}
                className="w-full h-[400px] p-6 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] font-mono text-[10px] leading-relaxed resize-none outline-none focus:ring-2 focus:ring-indigo-500/20 break-all dark:text-slate-600"
              />
              <div className="absolute bottom-4 right-4 text-[10px] font-bold text-slate-600 bg-white/80 dark:bg-slate-800/80 backdrop-blur px-3 py-1 rounded-full border border-slate-200 dark:border-slate-700">
