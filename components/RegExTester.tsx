...
self.onmessage = function(e) {
  ...
  try {
    ...
    const matches = [];
    const iterator = testText.matchAll(re);
...
export function RegExTester() {
  ...
  useEffect(() => {
    ...
    // ReDoS protection: timeout after 2 seconds
  ...
  const renderHighlightedText = () => {
    // Only render highlights if they match the current text to avoid layout shift while processing
    const currentTestText = testText;
    const isStale = lastProcessedText !== currentTestText;

    if (error || !regex || matches.length === 0 || isStale) {
      return <span className="text-transparent whitespace-pre-wrap break-words">{currentTestText}</span>;
    }
      ...
      if (start > lastIndex) {
        segments.push(
          <span key={`text-${i}`} className="text-transparent">
            {currentTestText.substring(lastIndex, start)}
          </span>
        );
      }
    ...
    if (lastIndex < currentTestText.length) {
      segments.push(
        <span key="text-end" className="text-transparent">
          {currentTestText.substring(lastIndex)}
        </span>
      );
    }
