    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {Object.entries(conversions).map(([name, converter]) => {
          ...
          return (
            <div key={name} className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 hover:border-indigo-500/30 transition-all group">
              <div className="flex items-center justify-between mb-3">
                <span className="font-bold text-xs uppercase tracking-wider text-slate-400 group-hover:text-indigo-500 transition-colors">{name}</span>
                <button
              ...
              <div className="bg-slate-50 dark:bg-slate-950/50 p-4 rounded-xl font-mono text-sm min-h-[4rem] break-all dark:text-slate-300 border border-transparent dark:border-slate-800/50">
