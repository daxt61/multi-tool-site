...
import { ImageIcon, Download, Trash2, Upload, Sliders, RefreshCw, AlertCircle, Info, Pipette, ShieldCheck } from 'lucide-react';
import { useTranslation } from 'react-i18next';

const MAX_SIZE = 5 * 1024 * 1024; // 5MB

export function TransparentImageMaker({ initialData, onStateChange }: { initialData?: any; onStateChange?: (state: any) => void }) {
  const { t } = useTranslation();
  const [image, setImage] = useState<string | null>(initialData?.image || null);
  const [targetColor, setTargetColor] = useState(initialData?.targetColor || '#ffffff');
  ...
  useEffect(() => {
    onStateChange?.({ image, targetColor, tolerance });
  }, [image, targetColor, tolerance, onStateChange]);
  ...
  }, [image, targetColor, tolerance, processImage]);
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="bg-slate-50 dark:bg-slate-900/50 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 space-y-8">
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 px-1">{t('transparent_image.color_to_remove', 'Color to make transparent')}</label>
                ...
                <p className="text-[10px] text-slate-400 italic px-1">{t('imagecolorreplacer.picker_hint')}</p>
              </div>

              <div className="space-y-2">
                <div className="flex justify-between items-center px-1">
                  <label htmlFor="tolerance" className="text-[10px] font-black uppercase tracking-widest text-slate-500">{t('imagecolorreplacer.tolerance')}</label>
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          <div className="relative min-h-[400px] bg-slate-50 dark:bg-slate-900 border-4 border-dashed border-slate-200 dark:border-slate-800 rounded-[2.5rem] flex items-center justify-center overflow-hidden group bg-[url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAMUlEQVQ4T2NkYGAQYcAP3uCTZhw1gGGYhAGth2H/G0S9uAEYIPXidS9yAb0f47Sj9DMANS4KAnPZ7+8AAAAASUVORK5CYII=')]">
            {image ? (
              <>
                ...
                <canvas
                  ...
                  title={t('imagecolorreplacer.click_to_pick')}
                />
                {isProcessing && (
              ...
              <label className="flex flex-col items-center gap-4 cursor-pointer p-12 text-center bg-white dark:bg-slate-950 rounded-3xl">
      ...
      <div className="bg-indigo-50 dark:bg-indigo-900/10 p-8 rounded-[2.5rem] border border-indigo-100 dark:border-indigo-900/20 flex items-start gap-4">
        ...
        <div className="space-y-2">
          <h4 className="font-bold dark:text-white">{t('transparent_image.about_title', 'About Transparent Image Maker')}</h4>
          <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
            {t('transparent_image.about_text', 'This tool allows you to remove a specific color from an image, making it transparent. It is particularly useful for creating transparent backgrounds for logos or icons. You can select the color to remove by using the color picker or by clicking directly on the image preview.')}
          </p>
