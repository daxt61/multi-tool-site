      ...
      <div className="mb-12 space-y-4">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleShare}
              className="flex items-center gap-2 px-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 rounded-2xl font-bold transition-all hover:bg-slate-100 dark:hover:bg-slate-800 active:scale-95"
              aria-label="Partager cet outil"
              title="Partager cet outil"
            >
              {shareFeedback ? <Check className="w-5 h-5 text-emerald-500" /> : <Share2 className="w-5 h-5" />}
              <span className="hidden sm:inline">{shareFeedback ? "Lien copié" : "Partager"}</span>
            </button>
            <button
