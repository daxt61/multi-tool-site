...
interface TSVToHTMLTableProps {
  initialData?: any;
  onStateChange?: (state: any) => void;
}

export const TSVToHTMLTable: React.FC<TSVToHTMLTableProps> = ({
  initialData,
  onStateChange,
}) => {
  ...
  // Generate HTML Table markup from TSV
  ...
  const handleCopy = () => {
    ...
    setCopied(true);
    toast.success(t('tsvtohtmltable.toast_copied', 'HTML table markup copied to clipboard!'));
  ...
  const handleClear = () => {
    ...
    toast.success(t('tsvtohtmltable.toast_cleared', 'Input cleared!'));
  };
  ...
  const presets = [
    {
      name: t('tsvtohtmltable.preset_catalog', 'E-Commerce Catalog'),
      data: `SKU\tProduct Name\tCategory\tPrice\tStock\nELE-101\tWireless Mouse\tElectronics\t$29.99\t150\nELE-102\tMechanical Keyboard\tElectronics\t$89.50\t45\nFUR-201\tErgonomic Chair\tFurniture\t$199.00\t12\nKIT-301\tStainless Water Bottle\tKitchen\t$15.00\t80`,
    },
    {
      name: t('tsvtohtmltable.preset_users', 'User Directory'),
    ...
    {
      name: t('tsvtohtmltable.preset_financial', 'Financial Report'),
    ...
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1 mr-2">
            ...
            {t('tsvtohtmltable.presets_title', 'Quick Presets:')}
          </span>
          {presets.map((preset, idx) => (
            <button
              ...
              onClick={() => {
                ...
                toast.success(t('tsvtohtmltable.toast_preset_loaded', 'Preset loaded!'));
              }}
      ...
      <div className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-1.5">
            <label
              htmlFor="tsv-html-custom-class"
              className="block text-xs font-bold text-slate-600 dark:text-slate-400"
            >
              {t('tsvtohtmltable.table_class', 'Table Class')}
            </label>
            <input
              id="tsv-html-custom-class"
              type="text"
              value={customClass}
          ...
          <div className="flex items-center justify-between sm:justify-start gap-3 pt-4 sm:pt-0">
            <label
              htmlFor="tsv-html-has-header"
              className="text-xs font-bold text-slate-600 dark:text-slate-400 cursor-pointer"
            >
              {t('tsvtohtmltable.has_header', 'First row is header')}
            </label>
            <input
              id="tsv-html-has-header"
              type="checkbox"
              checked={hasHeader}
        ...
        <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex flex-wrap items-center gap-6 text-xs font-semibold text-slate-600 dark:text-slate-400">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            {t('tsvtohtmltable.bordered', 'Bordered (.table-bordered)')}
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              ...
              className="w-3.5 h-3.5 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500 cursor-pointer"
            />
            {t('tsvtohtmltable.striped', 'Striped (.table-striped)')}
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              ...
              className="w-3.5 h-3.5 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500 cursor-pointer"
            />
            {t('tsvtohtmltable.hoverable', 'Hoverable (.table-hover)')}
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              ...
              className="w-3.5 h-3.5 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500 cursor-pointer"
            />
            {t('tsvtohtmltable.compact', 'Compact (.table-sm)')}
          </label>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label
              htmlFor="tsv-html-output"
              className="text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2"
            >
              <Code2 className="w-4 h-4 text-emerald-500" />
              {t('tsvtohtmltable.output_label', 'HTML Table Markup')}
            </label>
          ...
          <textarea
            ...
            value={output}
            placeholder={t(
              'tsvtohtmltable.placeholder_output',
      ...
      <div className="p-5 bg-indigo-50/50 dark:bg-indigo-950/20 rounded-2xl border border-indigo-100 dark:border-indigo-900/30 text-xs text-slate-600 dark:text-slate-400 space-y-2">
        <div className="flex items-center gap-2 font-bold text-indigo-900 dark:text-indigo-300">
          ...
          {t('tsvtohtmltable.about_title', 'About TSV to HTML Table Converter')}
        </div>
        <p className="leading-relaxed">
          {t(
            'tsvtohtmltable.about_text',
            'Convert TSV (tab-separated values) data directly into clean HTML <table> markup. Features HTML entity escaping, CSS table styling options (bordered, striped, hoverable, compact), pretty-print or minified output, and instant copy/download functionality. All processing happens entirely client-side in your browser for total privacy.'
          )}
        </p>
      </div>
    </div>
