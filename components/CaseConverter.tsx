    ...
    <div className="max-w-6xl mx-auto space-y-12">
      ...
      <div className="space-y-4">
        ...
        <textarea
          ...
          className="w-full h-40 p-6 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all text-lg leading-relaxed dark:text-slate-300"
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {Object.entries(conversions).map(([name, converter]) => {
          ...
          return (
            <div key={name} className="group p-6 bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-3xl hover:border-indigo-500/50 transition-all flex flex-col h-full">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  disabled={!text}
                  className={`p-2 rounded-xl transition-all ${copied === name ? 'bg-emerald-500 text-white' : 'text-slate-400 hover:text-indigo-500 bg-slate-50 dark:bg-slate-800'}`}
              ...
              <div className="flex-grow flex items-center">
                <div className="font-mono text-sm dark:text-slate-300 break-all line-clamp-3">
                  {converted || <span className="text-slate-400 italic">En attente de texte...</span>}
                </div>
              </div>
              <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="text-[9px] font-bold text-indigo-500 flex items-center gap-1">
                  Format {name} <ArrowRight className="w-2 h-2" />
                </span>
              </div>
            </div>
          );
