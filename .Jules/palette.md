## 2026-09-07 - [View Mode ARIA Toggle States, Action Toasts & Focus Management in Comparison Tools]
**Learning:** Comparison utilities (like Diff Checker) often omit `aria-pressed` state communication on view mode toggle buttons (e.g. Unified vs Split views), lack toast feedback on actions like swapping or downloading diffs, and drop user focus after clearing inputs. Supplying explicit `aria-pressed` attributes on view mode buttons, triggering `sonner` success toasts for copy, swap, download, and reset actions, and programmatically restoring focus to the primary original text input (`#text1`) ensures a seamless, accessible user flow.
**Action:** Always add `aria-pressed` to mode toggle buttons, trigger `sonner` toasts on output actions (swap/copy/download/reset), and restore focus to primary text inputs upon resetting.

## 2026-09-06 - [ARIA Slider Attributes, Focus Management & Container-Isolated Shortcuts in Password Generators]
**Learning:** Security generation tools (like Password Generator) often omit explicit ARIA value properties (`aria-valuemin`, `aria-valuemax`, `aria-valuenow`) on range slider controls, lack focus restoration to the primary configuration input on clear actions, and risk hotkey collisions when global keyboard shortcuts intercept navigation. Adding explicit ARIA slider bounds, restoring focus to `#password-quantity` on clear via `primaryInputRef`, and gating hotkeys with `containerRef.current?.contains(activeElement) || activeElement === document.body` provides a smooth, screen-reader friendly, WCAG-compliant micro-UX.
**Action:** Always provide explicit ARIA value attributes on range inputs, isolate hotkeys with `containerRef`, and restore focus to primary configuration controls on clear or reset actions.

## 2026-09-05 - [Interactive Presets, Animation ARIA States & Non-Disruptive Space Hotkeys in Bezier Generators]
**Learning:** Visual curve and animation generators (like Cubic Bezier Generator) often lack active `aria-pressed` states on preset buttons, localized `aria-label`s on play/pause controls, and container-scoped keyboard listener isolation. Intercepting `Space` key when focused on `document.body` breaks browser page scrolling. Requiring strict container bounds (`containerRef.current?.contains(activeElement)`) before intercepting `Space` preserves standard page scroll behavior while enabling fluid hotkeys (`Escape` reset & focus `#reset-button`, `C` copy CSS, `Space` play/pause preview when container focused) and Sonner toasts.
**Action:** Always provide interactive presets with `aria-pressed` states, isolate hotkey execution to `containerRef`, prevent `Space` key interception when focused on `document.body`, and use localized `aria-label`s.

## 2026-09-04 - [Interactive Presets, Form Associations & Hotkey Isolation in CSS Filter Generators]
**Learning:** CSS visual effect generators (like CSS Filter Generator) often lack interactive quick presets, explicit HTML label-to-input pairings (`htmlFor`/`id`), range slider ARIA properties (`aria-valuemin`, `aria-valuemax`, `aria-valuenow`), and container-scoped keyboard shortcuts. Adding quick start presets with `aria-pressed` attributes, binding form labels, triggering Sonner success toasts, gating hotkeys (`Escape` reset/focus & `C` copy) with `containerRef.current?.contains(activeElement)`, and restoring focus to the primary input on reset creates an intuitive, accessible micro-UX.
**Action:** Always provide interactive quick presets with `aria-pressed` states, connect range slider controls with explicit HTML label pairings, isolate keydown listeners to `containerRef`, and restore focus to `#filter-blur` on reset.
...
## 2026-08-14 - [Explicit Form-to-Text Associations & Quick Copy Actions in Color Tools]
**Learning:** Color manipulation utilities often bind `<label htmlFor="...">` to auxiliary input elements (like small `<input type="color">` squares) rather than primary text fields (`#foreground` and `#background`), causing label clicks to open color pickers unexpectedly instead of focusing text inputs. Binding labels to text inputs, pairing copy actions with global `sonner` toasts, providing an explicit contrast ratio copy target with visual `<Kbd>` key hint badges (`Esc`, `S`, `C`), and restoring focus via `useRef` on reset elevates accessibility and keyboard flow to WCAG AA compliance.
**Action:** Always ensure `<label htmlFor="...">` targets the main user-editable text input, provide discrete copy triggers for calculated results, and restore keyboard focus to the primary input upon reset.

## 2026-08-13 - [Bilingual Transfer Rate Estimators & Keyboard Focus Isolation]
**Learning:** Network transfer estimators (such as download time calculators) often suffer from hardcoded descriptions, lack of clipboard exports, and non-accessible layout flow. Unifying these tools under localized i18n keys, integrating custom inline copy controls accompanied by `sonner` success toasts, and wrapping global listeners using a `useRef` closure safeguard (`handlersRef` pattern) to correctly preserve and restore focus to primary inputs guarantees an incredibly polished, responsive, and WCAG-compliant experience.
**Action:** Always map network calculators with translation files, implement local focus-restoration targets on clearing, supply a visual copy summary action, and bind secure keyboard listeners to avoid input collision during user text entry.

## 2026-08-12 - [Bilingual Successive Discount Calculations & Focus Management]
**Learning:** Utilities that perform multi-step or successive calculations (such as successive discounts) are often hardcoded in a single language, lacking accessibility features and easy clipboard export controls. Offering full bilingual translation, `<Kbd>` key cap shortcut badges (Esc to clear, C to copy), programmatically restoring active focus on the primary input on clears, and displaying clear step-by-step calculations with localized `sonner` toasts guarantees a high-fidelity visual and interactive user experience.
**Action:** Always map successive calculation interfaces with translation collections, support programmatic focus restoration, include copy triggers with `sonner` success feedback, and secure shortcut hooks using the `handlersRef` safeguard.
