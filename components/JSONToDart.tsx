...
export function JSONToDart({ initialData, onStateChange }: { initialData?: any; onStateChange?: (state: any) => void }) {
  ...
  const toCamelCase = (str: string) => {
    ...
    let result = pascal.charAt(0).toLowerCase() + pascal.slice(1) || 'unnamedProperty';
    ...
    try {
      ...
      const getDartType = (val: any, fieldName: string, depth: number): string => {
        ...
        if (typeof val === 'object') {
          ...
          classStr += `\n\n  ${finalName}({\n`;
          classStr += Object.keys(val).map(key => {
            const ident = toCamelCase(key);
            return `    this.${ident},`;
          }).join('\n');
          classStr += '\n  });\n}';
      ...
      <div className="bg-indigo-50 dark:bg-indigo-900/10 p-8 rounded-[2.5rem] border border-indigo-100 dark:border-indigo-900/20 flex items-start gap-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
            Cet outil génère des classes Dart à partir de vos données JSON, incluant un constructeur nommé. C'est idéal pour le développement Flutter.
          </p>
        </div>
      </div>
    </div>
