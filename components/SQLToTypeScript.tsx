...
export function SQLToTypeScript({ initialData, onStateChange }: { initialData?: any; onStateChange?: (state: any) => void }) {
  ...
  const sanitizePropertyName = (name: string) => {
    // Sentinel: Ensure the property name is a valid TypeScript identifier or quote it.
    // This prevents snippet injection where a malicious column name could break the interface syntax.
    const isValidIdent = /^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(name);
    return isValidIdent ? name : `"${name.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`;
  };
  ...
  const handleConvert = useCallback(() => {
    try {
      ...
      while ((match = tableRegex.exec(input)) !== null) {
        ...
        const filteredLines = columnLines.filter(line => {
