...
## 2026-02-20 - [Navigation Affordance & Category Hierarchy]
**Learning:** Adding subtle animations (like an arrow sliding on hover) to navigation links significantly improves directional affordance and makes the UI feel more interactive. Additionally, resolving raw category IDs to localized names and icons in detail views ensures visual consistency with the main dashboard and improves accessibility for all users.
**Action:** Use the `group` and `group-hover` pattern for directional animations and always resolve category metadata from a central config for localized tool headers.
