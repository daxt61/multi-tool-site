...
import { Tag, Percent, Banknote, Trash2, Info, ShoppingCart, TrendingDown, Copy } from 'lucide-react';
...
export function DiscountCalculator() {
  ...
  const handleClear = () => {
    ...
    toast.success(t('discount.cleared_toast'));
  };

  const handleCopy = () => {
    if (!result) return;
    const originalVal = parseFloat(price) || 0;
    const originalFmt = originalVal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    const finalFmt = result.finalPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    const savingsFmt = result.totalSavings.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    const percentFmt = result.totalDiscountPercent.toFixed(1);

    const text = t('discount.copied_text_format', {
    ...
    toast.success(t('discount.copied_toast'));
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12" data-testid="discount-calculator-container">
        ...
        <div className="lg:col-span-6 space-y-8 bg-slate-50 dark:bg-slate-900/50 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800">
          <div className="flex justify-between items-center px-1">
            <h3 className="text-xs font-black uppercase tracking-widest text-slate-400">
              {t('discount.params_title')}
            </h3>
            <button
              onClick={handleClear}
              aria-label={`${t('discount.clear')} (Escape)`}
              className="text-xs font-bold text-rose-500 bg-rose-50 dark:bg-rose-500/10 hover:bg-rose-100 transition-all px-3 py-1.5 rounded-xl flex items-center gap-1.5"
            >
              <Trash2 className="w-3 h-3" aria-hidden="true" />
              <span>{t('discount.clear')}</span>
              <Kbd modifier={null} className="text-rose-500/80 border-rose-200 dark:border-rose-900/40 bg-rose-50/50">Esc</Kbd>
            </button>
          </div>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <label htmlFor="price" className="text-xs font-bold text-slate-500 dark:text-slate-400 px-1 flex items-center gap-2">
                <Banknote className="w-3 h-3" aria-hidden="true" /> {t('discount.original_price')}
              </label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label htmlFor="discount1" className="text-xs font-bold text-slate-500 dark:text-slate-400 px-1 flex items-center gap-2">
                  <Percent className="w-3 h-3" aria-hidden="true" /> {t('discount.discount1')}
                </label>
                <div className="relative">
                  <input
                    id="discount1"
              ...
              <div className="space-y-2">
                <label htmlFor="discount2" className="text-xs font-bold text-slate-500 dark:text-slate-400 px-1 flex items-center gap-2">
                  <Percent className="w-3 h-3" aria-hidden="true" /> {t('discount.discount2')} <span className="text-[10px] opacity-50 ml-1">{t('discount.optional')}</span>
                </label>
                <div className="relative">
                  <input
                    id="discount2"
        ...
        <div className="lg:col-span-6 space-y-6">
          <div className="bg-slate-900 dark:bg-black p-10 rounded-[2.5rem] shadow-xl shadow-indigo-500/10 flex flex-col items-center justify-center space-y-4 relative overflow-hidden text-center">
            ...
            {result && (
              <button
                onClick={handleCopy}
                aria-label={`${t('discount.copy_btn')} (C)`}
                className="absolute top-6 right-6 text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 transition-all p-2.5 rounded-xl flex items-center gap-1.5 text-xs font-bold"
              >
                <Copy className="w-4 h-4" aria-hidden="true" />
                <Kbd modifier={null} className="text-slate-300 border-slate-700 bg-slate-800">C</Kbd>
              </button>
            )}

            <div className="text-slate-400 font-bold uppercase tracking-widest text-xs">{t('discount.final_price')}</div>
            <div className="text-6xl md:text-7xl font-black text-white font-mono tracking-tighter">
              {result ? result.finalPrice.toLocaleString(i18n.language === 'en' ? 'en-US' : 'fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "0,00"}
            </div>
            <div className="text-emerald-400 font-black text-xl md:text-2xl uppercase tracking-widest flex items-center gap-2">
              <ShoppingCart className="w-5 h-5" aria-hidden="true" /> {t('discount.euros')}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/20 p-6 rounded-3xl space-y-2 text-center">
              <div className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest flex items-center justify-center gap-2">
                <TrendingDown className="w-3 h-3" aria-hidden="true" /> {t('discount.total_savings')}
              </div>
              <div className="text-2xl font-black text-emerald-600 dark:text-emerald-400 font-mono">
                -{result ? result.totalSavings.toLocaleString(i18n.language === 'en' ? 'en-US' : 'fr-FR', { minimumFractionDigits: 2 }) : "0,00"}€
              </div>
            </div>
            <div className="bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-900/20 p-6 rounded-3xl space-y-2 text-center">
              <div className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest flex items-center justify-center gap-2">
                <Percent className="w-3 h-3" aria-hidden="true" /> {t('discount.total_discount')}
              </div>
          ...
          {result && discount2 && parseFloat(discount2) > 0 && (
            <div className="bg-white dark:bg-slate-900/40 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-4">
              <h4 className="text-xs font-black uppercase tracking-widest text-slate-400">{t('discount.details_title')}</h4>
              <div className="space-y-3">
                <div className="flex justify-between items-center text-sm font-medium">
                  <span className="text-slate-500">{t('discount.initial_price')}</span>
                ...
                <div className="flex justify-between items-center text-sm font-medium text-emerald-500">
                  <span>{t('discount.remise1_label', { discount: discount1 })}</span>
                ...
                <div className="flex justify-between items-center text-sm font-bold border-t border-slate-100 dark:border-slate-800 pt-2">
                  <span className="text-slate-400">{t('discount.subtotal')}</span>
                  <span className="font-mono dark:text-white">{result.intermediatePrice.toFixed(2)}€</span>
                </div>
                <div className="flex justify-between items-center text-sm font-medium text-emerald-500">
                  <span>{t('discount.remise2_label', { discount: discount2 })}</span>
      ...
      <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/20 p-8 rounded-[2.5rem] flex items-start gap-4">
        ...
        <div className="space-y-2">
          <h4 className="font-bold text-amber-900 dark:text-amber-100">{t('discount.info_title')}</h4>
          <p className="text-sm text-amber-800/80 dark:text-amber-400/80 leading-relaxed">
            {t('discount.info_desc')}
          </p>
        </div>
      </div>
    </div>
