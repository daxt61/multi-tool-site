...
## 2025-08-15 - [Tag Breakout in Go Struct Generators]
...
## 2026-09-02 - [String Interpolation and Single Quote Breakout in Dart Code Generator]
**Vulnerability:** The JSONToDart component was vulnerable to string interpolation breakout and syntax breakout in generated Dart data model serialization/deserialization code. Unescaped single quotes (`'`), dollar signs (`$`), backslashes (`\`), and control characters (`\n`, `\r`, `\t`) in user-provided JSON keys allowed breaking out of single-quoted string literals in Dart `fromJson` and `toJson` methods.
**Learning:** Dart processes string interpolation (`$variable` or `${expression}`) inside string literals, and unescaped single quotes inside single-quoted strings terminate the literal context, allowing arbitrary code or syntax injection when generated models are used in client projects.
**Prevention:** Always escape backslashes (`\\\\`), single quotes (`\\'`), dollar signs (`\\$`), and control characters (`\\n`, `\\r`, `\\t`) when interpolating dynamic key strings into generated Dart string literals.

## 2026-09-05 - [Delimiter and Comment Breakout in .properties Generator]
