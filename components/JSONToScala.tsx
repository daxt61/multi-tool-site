    ...
    try {
      ...
      const getScalaType = (val: any, fieldName: string, depth: number): string => {
        ...
        if (Array.isArray(val)) {
          ...
          const itemType = getScalaType(val[0], fieldName, depth + 1);
      ...
      const result = caseClasses.reverse().join('\n\n');
      setOutput(result);
      setError('');
    } catch (e: any) {
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="bg-indigo-50 dark:bg-indigo-900/10 p-8 rounded-[2.5rem] border border-indigo-100 dark:border-indigo-900/20 flex items-start gap-4">
        ...
        <div className="space-y-2">
          <h4 className="font-bold dark:text-white">{t('jsontoscala.about_title')}</h4>
          <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
            {t('jsontoscala.about_text')}
          </p>
        </div>
      </div>
    </div>
