...
import { ShieldCheck, Copy, Check, Trash2, AlertCircle, Download, Info, Database, Sparkles } from 'lucide-react';
...
export function SQLToZod({ initialData, onStateChange }: { initialData?: any; onStateChange?: (state: any) => void }) {
  ...
  const [nullabilityMode, setNullabilityMode] = useState<'nullable' | 'optional' | 'nullish'>(initialData?.nullabilityMode || 'nullable');
  const [useCoerce, setUseCoerce] = useState(initialData?.useCoerce || false);
  const [strictObjects, setStrictObjects] = useState(initialData?.strictObjects || false);
  const [exportTypes, setExportTypes] = useState(initialData?.exportTypes !== false);
  const [error, setError] = useState('');
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    onStateChange?.({ input, output, outputStyle, casing, nullabilityMode, useCoerce, strictObjects, exportTypes });
  }, [input, output, outputStyle, casing, nullabilityMode, useCoerce, strictObjects, exportTypes, onStateChange]);
...
CREATE TABLE comments (
  ...
  const mapSqlTypeToZod = (columnName: string, sqlType: string): string => {
    ...
    if (colNameLower.includes('url') || colNameLower.includes('website')) {
      return 'z.string().url()';
    }

    if (type.includes('INT') || type.includes('SERIAL')) {
      return useCoerce ? 'z.coerce.number().int()' : 'z.number().int()';
    }

    if (type.includes('FLOAT') || type.includes('DOUBLE') || type.includes('DECIMAL') || type.includes('NUMERIC') || type.includes('REAL')) {
      return useCoerce ? 'z.coerce.number()' : 'z.number()';
    }

    if (type.includes('BOOL') || type.includes('BIT')) {
      return useCoerce ? 'z.coerce.boolean()' : 'z.boolean()';
    }

    if (type.includes('DATE') || type.includes('TIME') || type.includes('TIMESTAMP')) {
      return useCoerce ? 'z.coerce.date()' : 'z.date()';
    }
  ...
  const handleConvert = useCallback(() => {
    try {
      ...
      let match;
      const tables: { tableName: string; columns: { rawName: string; zodPropName: string; zodExpr: string; isNullable: boolean }[] }[] = [];
      ...
      while ((match = tableRegex.exec(cleanInput)) !== null) {
        ...
        const columns = filteredLines.map(line => {
          ...
          const zodPropName = transformCase(rawName, casing);
          const zodBase = mapSqlTypeToZod(rawName, sqlType || 'VARCHAR');

          let zodExpr = zodBase;
          if (isNullable) {
            if (nullabilityMode === 'nullable') {
              zodExpr += '.nullable()';
            } else if (nullabilityMode === 'optional') {
              zodExpr += '.optional()';
            } else {
              zodExpr += '.nullish()';
            }
        ...
        }).filter(Boolean) as { rawName: string; zodPropName: string; zodExpr: string; isNullable: boolean }[];
      ...
      if (tables.length === 0) {
        setError(t('sqltozod.no_tables_found', 'No valid CREATE TABLE statements found.'));
        setOutput('');
        return;
      }
      ...
      tables.forEach(table => {
        ...
        generated += `export const ${schemaName} = z.object({\n`;
        table.columns.forEach(col => {
          const isValidIdent = /^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(col.zodPropName);
          const keyStr = isValidIdent ? col.zodPropName : JSON.stringify(col.zodPropName);
          generated += `  ${keyStr}: ${col.zodExpr},\n`;
        });
        generated += `})`;
        ...
        if (exportTypes && outputStyle === 'full') {
          generated += `export type ${typeName} = z.infer<typeof ${schemaName}>;\n`;
        }
      ...
      setError(t('sqltozod.error_parsing', 'Error parsing SQL DDL') + ': ' + e.message);
      setOutput('');
    }
  }, [input, outputStyle, casing, nullabilityMode, useCoerce, strictObjects, exportTypes, t]);
  ...
  const handleCopy = useCallback(() => {
    ...
    setCopied(true);
    toast.success(t('sqltozod.toast_copied', 'Zod schemas copied to clipboard!'));
  ...
  const handleClear = useCallback(() => {
    ...
    setOutput('');
    setError('');
    toast.success(t('sqltozod.toast_cleared', 'Inputs cleared!'));
  ...
  const loadPreset = (presetKey: keyof typeof PRESETS) => {
    ...
    toast.success(t('sqltozod.preset_loaded', 'Loaded SQL preset!'));
  };
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">
            {t('sqltozod.presets_title', 'Quick Start Presets:')}
          </span>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            {t('sqltozod.preset_ecommerce', 'E-Commerce Catalog')}
          </button>
          <button
            onClick={() => loadPreset('user_auth')}
            className="px-3 py-1.5 text-xs font-bold bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-indigo-500 rounded-xl transition-all shadow-sm focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none"
          >
            {t('sqltozod.preset_user_auth', 'User Auth & Roles')}
          </button>
          <button
            onClick={() => loadPreset('blog')}
            className="px-3 py-1.5 text-xs font-bold bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-indigo-500 rounded-xl transition-all shadow-sm focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none"
          >
            {t('sqltozod.preset_blog', 'Blog Posts & Comments')}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-5 bg-white dark:bg-slate-900/60 rounded-2xl border border-slate-200 dark:border-slate-800">
        <div className="space-y-1.5">
          <label htmlFor="zod-casing" className="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">
            {t('sqltozod.field_casing', 'Property Casing')}
          </label>
          <select
            id="zod-casing"
            value={casing}
        ...
        <div className="space-y-1.5">
          <label htmlFor="zod-nullability" className="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">
            {t('sqltozod.nullability_mode', 'Nullability Mode')}
          </label>
          <select
            id="zod-nullability"
            ...
            className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-semibold outline-none focus:ring-2 focus:ring-indigo-500"
          >
            <option value="nullable">.nullable() (T | null)</option>
            <option value="optional">.optional() (T | undefined)</option>
            <option value="nullish">.nullish() (T | null | undefined)</option>
          </select>
        </div>

        <div className="space-y-1.5">
          <label htmlFor="zod-style" className="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">
            {t('sqltozod.output_style', 'Output Style')}
          </label>
          <select
            id="zod-style"
            value={outputStyle}
            ...
            className="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-semibold outline-none focus:ring-2 focus:ring-indigo-500"
          >
            <option value="full">{t('sqltozod.style_full', 'Full File (Imports + Types)')}</option>
            <option value="schema_only">{t('sqltozod.style_schemas', 'Schemas Only')}</option>
          </select>
        </div>
        ...
        <div className="flex flex-col justify-center space-y-2 pt-2">
          <label className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-300 cursor-pointer">
            <input
              type="checkbox"
              checked={useCoerce}
              onChange={(e) => setUseCoerce(e.target.checked)}
              className="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500"
            />
            {t('sqltozod.use_coerce', 'Use z.coerce for values')}
          </label>
          <label className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-300 cursor-pointer">
            <input
              type="checkbox"
              ...
              className="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500"
            />
            {t('sqltozod.strict_objects', 'Strict objects (.strict())')}
          </label>
        </div>
      </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-1">
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="sql-zod-input" className="text-xs font-black uppercase tracking-widest text-slate-400 cursor-pointer">
                {t('sqltozod.sql_input_label', 'SQL CREATE TABLE DDL')}
              </label>
            </div>
            <div className="flex items-center gap-2">
          ...
          <textarea
            ...
            placeholder={t('sqltozod.placeholder_sql', 'Paste SQL CREATE TABLE DDL statements here...')}
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-1">
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="zod-output" className="text-xs font-black uppercase tracking-widest text-slate-400 cursor-pointer">
                {t('sqltozod.output_label', 'Generated Zod Schemas')}
              </label>
            </div>
          ...
          <textarea
            ...
            value={output}
            readOnly
            placeholder={t('sqltozod.placeholder_output', 'Zod validation schema code will appear here...')}
      ...
      <div className="bg-indigo-50 dark:bg-indigo-900/10 p-8 rounded-[2.5rem] border border-indigo-100 dark:border-indigo-900/20 flex items-start gap-4">
        ...
        <div className="space-y-2">
          <h4 className="font-bold dark:text-white">{t('sqltozod.about_title', 'About SQL to Zod Schema Generator')}</h4>
          <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
            {t('sqltozod.about_text', 'Convert SQL CREATE TABLE DDL queries directly into strongly-typed Zod validation schemas (z.object({ ... })). The parser automatically infers column types, handles NOT NULL constraints, formats property casing, and generates TypeScript z.infer type exports.')}
          </p>
        </div>
      </div>
    </div>
