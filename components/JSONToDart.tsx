    ...
    try {
      ...
      const getDartType = (val: any, fieldName: string, depth: number): string => {
        ...
        if (typeof val === 'object') {
          ...
          const fields = Object.entries(val).map(([key, value]) => ({
            name: key,
            type: getDartType(value, key, depth + 1),
          }));
      ...
      let result = classes.reverse().map(cls => {
        ...
        cls.fields.forEach((f, idx) => {
          str += `this.${f.name}${idx === cls.fields.length - 1 ? '' : ', '}`;
        });
        str += '});\n\n';

        str += `  ${cls.name}.fromJson(Map<String, dynamic> json) {\n`;
        cls.fields.forEach(f => {
          str += `    ${f.name} = json['${f.name}'];\n`;
        });
        str += '  }\n\n';
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-1">
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="dart-output" className="text-xs font-black uppercase tracking-widest text-slate-400 cursor-pointer">{t('common.output')} Dart</label>
            </div>
            <button
              onClick={handleCopy}
              disabled={!output}
