...
import { Eye, Code, Columns, Info, ShieldCheck } from 'lucide-react';
...
export function MarkdownPreview() {
  ...
  const renderMarkdown = (text: string) => {
    ...
    html = html.replace(/```(?:[a-z]*)\n?([\s\S]*?)```/g, (_, code) => {
      ...
      placeholders.push(`<pre class="bg-slate-100 dark:bg-slate-800 p-4 rounded-lg my-4 font-mono text-sm overflow-x-auto text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-slate-700"><code>${code}</code></pre>`);
      return p;
    });
    ...
    html = html.replace(/`(.*?)`/g, (_, code) => {
      ...
      placeholders.push(`<code class="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-indigo-600 dark:text-indigo-400 font-mono text-sm border border-slate-200 dark:border-slate-700">${code}</code>`);
      return p;
    });
    ...
    // 4. Blockquotes
    html = html.replace(/^\s*&gt;\s+(.*)$/gim, '<blockquote class="border-l-4 border-indigo-500 dark:border-indigo-600 pl-4 italic my-6 text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 py-2 rounded-r-lg">$1</blockquote>');
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex justify-center">
        <div className="inline-flex bg-slate-100 dark:bg-slate-800 p-1 rounded-2xl border border-slate-200 dark:border-slate-700">
          {[
            ...
            { id: 'split', name: 'Divisé', icon: Columns },
      ...
      <div className={`grid gap-8 ${mode === 'split' ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1'}`}>
        {(mode === 'edit' || mode === 'split') && (
          <div className="space-y-4">
            ...
            <textarea
              ...
              className="w-full h-[500px] p-8 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all font-mono text-sm leading-relaxed dark:text-slate-300 resize-none"
        ...
        {(mode === 'preview' || mode === 'split') && (
          <div className="space-y-4">
             <div className="flex justify-between items-center px-1">
               <span id="preview-label" className="text-xs font-black uppercase tracking-widest text-slate-400">Rendu Visuel</span>
            </div>
            <div
              aria-labelledby="preview-label"
              className="w-full h-[500px] p-8 bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] overflow-y-auto prose dark:prose-invert max-w-none shadow-sm"
      ...
      <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-8 pt-16 border-t border-slate-100 dark:border-slate-800">
        <div className="space-y-4">
          <h4 className="font-bold dark:text-white flex items-center gap-2">
            <Info className="w-4 h-4 text-indigo-500" /> C'est quoi le Markdown ?
          </h4>
          <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
            Markdown est un langage de balisage léger qui permet d'écrire du texte formaté à l'aide d'une syntaxe simple en texte brut. Il est largement utilisé par les développeurs et les rédacteurs.
          </p>
        </div>
        <div className="space-y-4">
          <h4 className="font-bold dark:text-white flex items-center gap-2">
            <Code className="w-4 h-4 text-indigo-500" /> Syntaxe de base
          </h4>
          <div className="text-xs font-mono bg-slate-50 dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-1 text-slate-600 dark:text-slate-400">
            <div># Titre 1</div>
            <div>## Titre 2</div>
            <div>**Gras**</div>
            <div>*Italique*</div>
        ...
        <div className="space-y-4">
          <h4 className="font-bold dark:text-white flex items-center gap-2">
            <ShieldCheck className="w-4 h-4 text-indigo-500" /> Sécurisé & Privé
          </h4>
          <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
            Votre texte ne quitte jamais votre navigateur. Le rendu HTML est effectué localement et les scripts malveillants sont neutralisés avant l'affichage.
          </p>
        </div>
      </div>
