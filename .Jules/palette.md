...
## 2025-05-25 - [Modular Tool Branding Consistency]
**Learning:** Legacy or third-party style components (using `blue` instead of `indigo` or `gray` instead of `slate`) break the immersion of the "Bento" system. Consolidating these tools into the standard palette (`slate-50`, `indigo-600`) and using consistent corner radii (`rounded-3xl` for containers, `rounded-[2rem]` for inputs) restores brand cohesion and makes the application feel like a single, unified product rather than a collection of scripts.
**Action:** Audit and modernize color palettes and border radii in older components to match the primary design system.
