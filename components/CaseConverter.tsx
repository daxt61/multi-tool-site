    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {Object.entries(conversions).map(([name, converter]) => {
          ...
          return (
            <div key={name} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm hover:border-indigo-500/30 transition-all group">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                  <CaseSensitive className="w-3.5 h-3.5 text-indigo-500" />
                  {name}
                </span>
                <button
                  ...
                  className={`p-2 rounded-xl transition-all ${
                    copied === name
                      ...
                      : 'text-slate-400 hover:text-indigo-500 bg-slate-50 dark:bg-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-900/20'
                  } disabled:opacity-50 disabled:cursor-not-allowed`}
                  title="Copier"
                  aria-label={`Copier le texte en ${name}`}
                >
                  {copied === name ? (
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl font-mono text-sm min-h-[4rem] break-all border border-slate-100 dark:border-slate-700/50 dark:text-slate-300">
