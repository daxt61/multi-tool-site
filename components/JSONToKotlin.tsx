...
export function JSONToKotlin({ initialData, onStateChange }: { initialData?: any; onStateChange?: (state: any) => void }) {
  ...
  const escapeKotlinString = (str: string) => {
    ...
    // from the @JsonProperty attribute which could lead to snippet injection.
    return str
      .replace(/\\/g, '\\\\')
      .replace(/"/g, '\\"')
      .replace(/\n/g, '\\n')
      ...
      const getKotlinType = (val: any, fieldName: string, depth: number): string => {
        ...
        if (typeof val === 'object') {
          ...
          const fields = Object.entries(val).map(([key, value]) => {
            ...
            let ident = isValidIdent ? key : `\`${key.replace(/`/g, '')}\``;

            let annotation = '';
            if (!isValidIdent) {
              annotation = `    @JsonProperty("${escapeKotlinString(key)}")\n`;
            }

            return `${annotation}    val ${ident}: ${type}`;
          });
