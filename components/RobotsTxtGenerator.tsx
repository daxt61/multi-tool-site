...
import { Download, Trash2, FileSearch, Plus, X, Copy, Check, Info } from 'lucide-react';
...
export function RobotsTxtGenerator({ initialData, onStateChange }: { initialData?: any; onStateChange?: (state: any) => void }) {
  const [rules, setRules] = useState<Rule[]>(initialData?.rules || [
    { userAgent: '*', disallow: ['/cgi-bin/', '/admin/'], allow: ['/'] }
  ]);
  const [sitemap, setSitemap] = useState(initialData?.sitemap || '');
