...
export function RegExTester() {
  ...
  useEffect(() => {
    ...
    // Sentinel: Use a Web Worker with a timeout to prevent ReDoS (catastrophic backtracking)
    // from freezing the main UI thread.
    const workerCode = `
      self.onmessage = function(e) {
        ...
        try {
          ...
          for (const match of allMatches) {
            ...
            // Copy all numeric indices (0, 1, 2...)
    ...
    let timeoutId: any; // Type 'any' to avoid NodeJS.Timeout vs number conflict in browser build
    let executionTimeoutId: any;
    ...
    const runRegex = () => {
      ...
      executionTimeoutId = setTimeout(() => {
        ...
        setIsProcessing(false);
      }, 500); // 500ms timeout for complex regex
    };

    // Debounce to prevent excessive worker creation while typing
    timeoutId = setTimeout(runRegex, 200);
  ...
  const renderHighlightedText = () => {
    // Only render highlights if they match the current text to avoid visual jitter
    const activeMatches = isProcessing ? [] : matches;

    if (error || !regex || activeMatches.length === 0) {
    ...
    activeMatches.forEach((match, i) => {
      ...
      // Ensure we don't go out of bounds if text changed during processing
      if (start < lastIndex || end > testText.length) return;
