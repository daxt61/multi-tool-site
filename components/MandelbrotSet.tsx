...
import { Sparkles, ZoomIn, RotateCcw, Palette, Settings2, Download, Info } from 'lucide-react';
...
export function MandelbrotSet({ initialData, onStateChange }: { initialData?: any; onStateChange?: (state: any) => void }) {
  const { t } = useTranslation();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);

  const [centerX, setCenterX] = useState(initialData?.centerX ?? -0.5);
  ...
  useEffect(() => {
    onStateChange?.({ centerX, centerY, zoom, maxIter, palette });
  }, [centerX, centerY, zoom, maxIter, palette, onStateChange]);
  ...
  const draw = useCallback(() => {
    ...
    const renderChunk = () => {
      ...
      for (let py = y; py < endY; py++) {
        ...
        for (let px = 0; px < width; px++) {
          ...
          let x = 0, yVal = 0, iter = 0;
          while (x * x + yVal * yVal <= 4 && iter < maxIter) {
            const nextX = x * x - yVal * yVal + cx;
            yVal = 2 * x * yVal + cy;
            x = nextX;
            iter++;
          }
    ...
    renderRef.current = requestAnimationFrame(renderChunk);
  }, [centerX, centerY, zoom, maxIter, palette]);
      ...
      <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-12 pt-16 border-t border-slate-100 dark:border-slate-800">
        <div className="space-y-4">
          <h4 className="font-bold dark:text-white flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-indigo-500" /> {t('mandelbrot.what_is_title')}
          </h4>
          <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
            {t('mandelbrot.what_is_desc')}
          </p>
        </div>
        <div className="space-y-4">
          <h4 className="font-bold dark:text-white flex items-center gap-2">
            <ZoomIn className="w-4 h-4 text-indigo-500" /> {t('mandelbrot.infinite_title')}
          </h4>
          <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
            {t('mandelbrot.infinite_desc')}
          </p>
        </div>
        <div className="space-y-4">
          <h4 className="font-bold dark:text-white flex items-center gap-2">
