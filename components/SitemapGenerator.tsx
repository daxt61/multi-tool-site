...
export function SitemapGenerator({ initialData, onStateChange }: { initialData?: any; onStateChange?: (state: any) => void }) {
  ...
  const generateSitemap = () => {
    try {
      ...
      if (xml.length > MAX_LENGTH) {
        setError(t('error.max_length_sitemap', { max: MAX_LENGTH.toLocaleString() }));
        setOutput('');
        return;
      }
      ...
      setError(t('sitemap.error_generation') + ': ' + e.message);
    }
  };
  ...
  const addUrl = () => {
    if (urls.length >= MAX_URLS) {
      setError(t('sitemap.error_limit', { max: MAX_URLS }));
      return;
    }
  ...
  const handleBulkAdd = (text: string) => {
    ...
    if (lines.length + urls.length > MAX_URLS) {
      setError(t('sitemap.error_bulk_limit', { max: MAX_URLS }));
      return;
    }
    ...
    <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center px-1">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-black uppercase tracking-widest text-slate-400">{t('sitemap.config_title')}</h3>
            </div>
            <button
              onClick={addUrl}
            ...
            >
              <Plus className="w-3 h-3" /> {t('sitemap.add_url')}
            </button>
          </div>
            ...
            {urls.map((item, index) => (
              <div key={index} className="p-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-3 relative group animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t('sitemap.frequency')}</label>
                    <select
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t('sitemap.priority')} ({item.priority})</label>
                    <input
                      type="range"
                      min="0.0"
                      max="1.0"
          ...
          <div className="p-6 bg-indigo-50 dark:bg-indigo-900/10 rounded-2xl border border-indigo-100 dark:border-indigo-900/20 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-xs font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-400">{t('sitemap.bulk_add')}</h4>
            </div>
            <textarea
              placeholder={t('sitemap.bulk_placeholder')}
              onBlur={(e) => {
                if (e.target.value) {
      ...
      <div className="bg-slate-50 dark:bg-slate-900/50 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h4 className="font-bold text-slate-900 dark:text-white">{t('sitemap.about_title')}</h4>
        </div>
        <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
          {t('sitemap.about_text')}
        </p>
      </div>
    </div>
  );
