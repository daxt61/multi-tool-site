...
function MainApp() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 selection:bg-indigo-100 dark:selection:bg-indigo-900/50">
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-[100] focus:px-4 focus:py-2 focus:bg-indigo-600 focus:text-white focus:rounded-xl focus:font-bold focus:shadow-lg focus:shadow-indigo-500/20 transition-all"
      >
                  ...
                  <div className="flex gap-2 overflow-x-auto no-scrollbar">
                    {categories.map((cat) => (
                      <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold transition-all whitespace-nowrap border active:scale-95 ${
...
function ToolView({ favorites, toggleFavorite }: { favorites: string[], toggleFavorite: (e: React.MouseEvent, id: string) => void }) {
  ...
  const currentTool = useMemo(() => toolId ? toolsMap[toolId] : null, [toolId]);
  const category = useMemo(() => categories.find(c => c.id === currentTool?.category), [currentTool]);
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-full text-xs font-bold uppercase tracking-widest">
              {category?.icon ? <category.icon className="w-3 h-3" /> : <currentTool.icon className="w-3 h-3" />}
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 px-4 sm:px-6 py-3 rounded-2xl font-bold transition-all border active:scale-95 ${
