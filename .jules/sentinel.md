...
## 2026-02-20 - [XSS in RegEx Highlighter]
**Vulnerability:** The RegEx Tester used `dangerouslySetInnerHTML` to render highlighted matches. In error or empty states, it returned the raw `deferredText`, allowing for XSS if the input contained malicious scripts.
**Learning:** When using `dangerouslySetInnerHTML` for a "backdrop" or "preview" layer, every possible code path must ensure the input is escaped. Fallback paths are often overlooked but are just as vulnerable.
**Prevention:** Centralize the escape function and apply it to all string segments before they are combined into the final HTML string.
