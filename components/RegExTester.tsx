...
// Sentinel: Move regex execution to a Web Worker to prevent ReDoS (Regular Expression Denial of Service)
// from freezing the main thread. Pathological regexes like (a+)+$ can cause catastrophic backtracking.
const WORKER_CODE = `
  self.onmessage = function(e) {
    ...
    try {
      ...
      for (const match of matchAll) {
        // Sentinel: Limit match count to prevent memory issues and UI lag
        if (count >= 1000) break;
        matches.push({
          index: match.index,
          text: match[0],
      ...
      self.postMessage({ matches, matchCount: count, error: null, processedText: testText });
    } catch (e) {
      self.postMessage({ matches: [], matchCount: 0, error: e.message, processedText: testText });
    }
  };
`;
...
export function RegExTester() {
  ...
  useEffect(() => {
    ...
    // Note: We don't clear matches here to avoid flickering,
    // but we use lastProcessedText to ensure highlights match the current text.

    workerRef.current = new Worker(workerUrlRef.current!);

    workerRef.current.onmessage = (e) => {
      const { matches, matchCount, error, processedText } = e.data;
      setMatches(matches);
    ...
    // Sentinel: Set a timeout to terminate the worker if it takes too long (ReDoS protection)
  ...
  const renderHighlightedText = () => {
    // Sentinel: Only render highlights if they were calculated for the current text
    // to prevent misaligned highlights while typing.
    if (error || !regex || matches.length === 0 || lastProcessedText !== testText) {
