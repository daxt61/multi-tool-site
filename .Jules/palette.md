...
## 2026-03-06 - [Modernizing Utility Tools & Accessibility]
**Learning:** For text conversion tools, a "Clear" button is a critical micro-UX that improves efficiency. Additionally, ensuring all form fields have explicit `id`/`htmlFor` labels and descriptive ARIA labels for icon-only buttons is essential for a baseline accessible experience. Modernizing legacy components to the "Bento" design system (rounded-2xl/3xl, slate backgrounds, indigo accents) significantly enhances visual harmony.
**Action:** Always include a clear button for text inputs and verify label associations in utility tools. Use the Bento pattern for any UI modernization.
