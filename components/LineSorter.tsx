...
import { ArrowUpDown, Copy, Check, Download, Info, RotateCcw, SortAsc, Sparkles } from 'lucide-react';
...
const PRESETS: Preset[] = [
  {
    id: 'fruits',
    name: 'Unsorted Fruit Names',
    sortType: 'alpha-asc',
    sampleInput: `Watermelon
Apple
Banana
Orange
Mango
...
Peach`
  ...
  {
    id: 'ips',
    name: 'Mixed Server IPs',
    sortType: 'natural-asc',
    sampleInput: `192.168.1.105
10.0.0.1
192.168.1.2
172.16.0.10
10.0.0.20
...
-99`
  },
  {
    id: 'timestamps',
    name: 'Log Timestamps',
    sortType: 'natural-desc',
    sampleInput: `2025-05-01 10:15:00
2025-05-01 08:30:12
2025-05-02 14:22:00
2025-04-28 19:45:30
2025-05-01 11:00:00`
...
export function LineSorter({ initialData, onStateChange }: { initialData?: any; onStateChange?: (state: any) => void }) {
  ...
  const handleDownload = useCallback(() => {
    ...
    toast.success(t('linesorter.toast_downloaded', 'Exported sorted lines file!'));
  }, [sortedOutput, t]);
  ...
  const handlePresetSelect = useCallback((preset: Preset) => {
    ...
    setError(null);
    toast.success(t('linesorter.toast_preset_loaded', 'Loaded preset: {{name}}', { name: preset.name }));
  }, [t]);
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] p-6 space-y-3">
        <div className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-400">
          ...
          {t('linesorter.presets_title', 'Quick Presets')}
        </div>
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="flex justify-between items-center px-1">
            <label htmlFor="lines-input" className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-2 cursor-pointer">
              <ArrowUpDown className="w-4 h-4 text-indigo-500" aria-hidden="true" /> {t('linesorter.input_label', 'Lines to Sort')}
            </label>
            <button
              onClick={handleClear}
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center px-1">
              <label htmlFor="lines-output" className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                <SortAsc className="w-4 h-4 text-emerald-500" aria-hidden="true" /> {t('common.result', 'Result')}
              </label>
